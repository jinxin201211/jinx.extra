...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let that = this;
      this.axios
        .post("/api/gameWorks2/GetUploadInfo_GameType", qs.stringify({ sDate: kssj, eDate: jssj }))
          ...
          if (response && response.data.code == "0") {
            ...
            for (let i = 0; i < that.DateList.length; i++) {
              ...
              for (let j = 0; j < that.UserGroupCode.length; j++) {
                let stat = data.find(p => p.date === that.DateList[i] && p.game_type === that.UserGroupCode[j]);
                if (stat != null) {
